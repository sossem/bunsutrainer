...
(function (root) {
  ...
  function create({ esc: E, button, notify, onChange, onEnter }) {
    ...
    function authenticate(type) {
      ...
      const pin = state.pin;
      if (!/^\d{6}$/.test(pin) || (type === 'register' && pin !== state.pinConfirm)) { message('숫자 6자리 PIN을 입력하고, 확정할 때는 두 PIN을 같게 입력해 주세요.', true); onChange(); return; }
    ...
    function row(value, position, own) {
      ...
      return `<li class="competition-rank-row${ours ? ' competition-rank-own' : ''}"><span class="competition-position">${position === null ? '목록 밖' : position <= 3 ? `<span class="rank-medal" role="img" aria-label="${position}위">${['🥇','🥈','🥉'][position-1]}</span>` : number(position)}</span><span class="competition-rank-name"><strong>${E(title)}</strong><small>${E(subtitle || '')}${ours ? (state.kind === 'class' ? ' · 우리 반' : ' · 나') : ''}</small></span><span class="competition-rank-score">${number(value.score)}<small>점</small></span></li>`;
    }
    function board(week) {
