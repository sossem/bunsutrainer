...
def run():
    ...
    with browser as b:
        ...
        check("personal completion retains untimed score and record", record['completed'] and record['firstCorrect']==5 and 2250 < record['score'] <= 2750 and not record['timerEnabled'] and record['timerSeconds']==0,record)
        ...
        check("history reload preserves competition contribution",b.eval("document.querySelector('.record-growth').textContent.includes('랭킹 반영 완료') && LearningStorage.list()[0].score>2250 && LearningStorage.list()[0].score<=2750"))
        configure("personal", grade=4, unit="g4-addsub", kind="all", difficulty="normal")
        b.viewport(390,844)
        for index in range(5):
            answer(index)
            if index == 0:
                check("mixed practice awards advertised base plus first-attempt and bounded speed bonus",b.eval("Number(document.querySelector('.score-burst strong').textContent.replace(/[^0-9]/g,''))>1050 && Number(document.querySelector('.score-burst strong').textContent.replace(/[^0-9]/g,''))<=1550"))
                ...
                check('reduced motion hides transient burst but keeps earned score',b.eval("getComputedStyle(document.querySelector('.score-burst')).display==='none' && document.querySelector('.session-growth').textContent.includes('이번 문제 +')"))
                b.command('Emulation.setEmulatedMedia', {'features':[]})
            click('next')
        b.wait_for("LearningStorage.list()[0]?.classContributionStatus==='synced'")
        check('mixed practice large total survives saving and ranking submission',b.eval("LearningStorage.list()[0].score>6250 && LearningStorage.list()[0].score<=8750 && LearningStorage.list()[0].classScoreContribution===LearningStorage.list()[0].score && LearningStorage.list()[0].scoringVersion===3"))
