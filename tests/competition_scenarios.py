...
def run():
    ...
    with browser as b:
        ...
        field(b, '#competition-grade', 5)
        field(b, '#competition-class', 2)
        click('competition-class-select')
        check('selected school grade and class persist', b.eval("ClassRanking.getSelection().grade===5 && ClassRanking.getSelection().className==='2'"))
        ...
        repeated = b.eval('WeeklyCompetition.registerNickname(ClassRanking.getSelection(),' + json.dumps(nickname) + ')')
        ...
        check('login binds selected class and nickname', identity['nickname'] == nickname and identity['classroom']['grade'] == 5 and identity['classroom']['className'] == '2')
        ...
        check('student can choose all three difficulties with differentiated scores', b.eval("document.querySelectorAll('[data-action=difficulty]').length===3 && document.querySelector('[data-action=difficulty][data-value=easy]').textContent.includes('10점') && document.querySelector('[data-action=difficulty][data-value=challenge]').textContent.includes('13점')"))
        for width, height in [(1920, 1080), (1366, 768), (768, 1024), (390, 844)]:
        ...
        check('difficulty changes earned scores using existing rules', scores == [66, 71, 81], scores)
        total = sum(scores)
